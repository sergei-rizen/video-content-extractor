    ...
    for file_entry in files_to_process_now:
        ...
        if download_file_from_dropbox(dbx, dropbox_watch_file_path, local_temp_video_path):
            ...
            try:
                ...
                # Use the terminal state values for checking
                terminal_states = [genai.File.State.SUCCEEDED, genai.File.State.FAILED, genai.File.State.CANCELLED]

                # Refresh file object status until processed - CHECK THE STATE VALUE DIRECTLY
                # This loop will now correctly wait until the state is one of the terminal values
                while file_obj.state not in terminal_states:
                ...
                # After the loop, file_obj.state IS one of the terminal_states (SUCCEEDED, FAILED, CANCELLED)
                # Check the final processing state based on the state value
                if file_obj.state == genai.File.State.SUCCEEDED:
                    print(f"Gemini processing succeeded for {file_name}.")
                    # Content generation follows this block
                elif file_obj.state == genai.File.State.FAILED:
                ...
                elif file_obj.state == genai.File.State.CANCELLED:
